...
def view_logs_and_article(username:str, log_dir:str):
    ...
    return render_template('response.html', log_url=None, article_url=None, showing=True, markdown_left=read_markdown_to_html(objs.first().file.read().decode('utf-8')), markdown_right=read_markdown_to_html(objs.first().file.read().decode('utf-8')))
...
def write(username:str, log_dir:str, query:str, model:str, max_width:int, max_depth:int, n_tokens:int, api_key:str):
    process_url = url_for('bp_processing_api.process', query=query, log_dir=log_dir, model=model, max_width=max_width, max_depth=max_depth, n_tokens=n_tokens, api_key=api_key, _external=True)
    return render_template('response.html', depth=max_depth, log_url=process_url, article_url=None, showing=False)
...
def write_article(username:str, log_dir:str, n_tokens:int, api_key:str, n_iterations:int):
    article_url = url_for('bp_processing_api.create_article', username=username, log_dir=log_dir, model=thinker.model, n_token_per_iteration=n_tokens, api_key=api_key, _external=True)
    return render_template('response.html', n_iterations=n_iterations, article_url=article_url, log_url=None, showing=False)
