...
def process(query, log_dir, model, max_width, max_depth, n_tokens, api_key):
    ...
    if obj_context.file is None:
        obj_context.file.put(" ".encode("utf-8"), content_type="text/markdown")
...
@bp_processing_api.route('/create_article?username=<username>&log_dir=<log_dir>&model=<model>&iteration=<int:iteration>&api_key=<api_key>&n_tokens_per_iteration=<n_tokens_per_iteration>', methods=['GET'])
def create_article(username, log_dir, model, iteration, api_key, n_tokens_per_iteration):
    thinker = Reasoning("", 0, 0)
    obj_response = Upload.objects(filename__contains=os.path.join(log_dir, 'response.md')).first()
    obj_context = Upload.objects(filename__contains=os.path.join(log_dir, 'context.md')).first()
    thinker.n_tokens_default = n_tokens_per_iteration
